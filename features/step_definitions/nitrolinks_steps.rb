...
Then(/^the '([^']*)' page should be loaded from cache$/) do |name|
  wait_for_page_load
  expect_nitro_page_restored(name).to eql(true), "Page #{name} (#{nitro_page(name)}) has not been restored from cache"
  expect_page_content_to_be_loaded(name)
end
...
Then(/^The '([^']*)' page should not be loaded in any way$/) do |name|
